...
def redact_text(text, vault, known=(), model=None, local_call=ollama_json,
    ...
    entities = list(known)
    for kind, pattern in [
        # Credentials inside a URI are unambiguous, and masking the whole thing
        # first stops the email pattern carving a marker out of its middle,
        # which leaves the model no valid span to return.
        ('SECRET', r'[a-zA-Z][\w+.-]*://[^\s:@/]+:[^\s@/]+@[^\s\'"<>]+'),
        ('URL', r'https?://[^\s<>"\']+'),
        ('PATH', r'(?<=")/[^"\n]+(?=")'),
        ('PATH', r"(?<=')/[^'\n]+(?=')"),
        ('PATH', r'(?<![\w:/])/(?:[^\s<>"\x27,;]+)'),
    ...
    for attempt in range(2):
        ...
        if isinstance(response, dict) and set(response) == {'entities'} and isinstance(response['entities'], list):
            ...
            for candidate in response['entities']:
                if not (isinstance(candidate, dict) and isinstance(candidate.get('text'), str)):
                    candidates.append(candidate)
                    continue
                if candidate['text'].casefold() in marker_names:
                    continue
                pieces = [candidate['text']]
                for marker in model_markers:
                    pieces = [piece for part in pieces
                              for piece in re.split(re.escape(marker), part, flags=re.IGNORECASE)]
                if len(pieces) == 1:
                    candidates.append(candidate)
                    continue
                # A span wrapped around a marker is not a hallucination: it is
                # the real private text on either side, which is what the
                # marker rule asks for. Keep those fragments.
                for piece in pieces:
                    piece = piece.strip(' :,;"\'')
                    if len(piece) >= 3:
                        candidates.append({'text': piece, 'kind': candidate.get('kind', 'VALUE')})
